...
- It is not allowed to update value of "entry_id" once record is created - if happens exception is thrown and operation is rolled back
- Value of "name" is trimmed (with notification about it in case this changes value)
- (Trimmed) value of "name" cannot be same as "main_name" of a corresponding "entry" in "musical_entries" table - if happens exception is thrown and operation is rolled back.
